...
for (i in 1:n_sim_h0) {
  G = sample_grg(nodes = vertices, radius = coef.h0, torus = TRUE, coords = FALSE)
  G = asNetwork(G)
  g.sim[[i]] <- G
}
...
powertest.parallel <- function(coef_idx, kernel_idx, B_idx) {
  ...
  coef.h1 = coef
  g.sim.h1 <- lapply(rep(vertices, n_sim_h1), sample_grg, radius = coef.h1, torus = TRUE, coords = FALSE)
...
setwd("..")
setwd("Data")
#setwd("Geometric RG Experiment")
ggsave("GRGcomnb_all_v20_B200_n500.jpg", plot = powerplot)
write.csv(df,"GRGcomnb_all_v20_B200_n500.csv", row.names = FALSE)