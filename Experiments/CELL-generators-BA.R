...
for (i in (1:n_sim_h0)) {
  G = sample_pa(n = vertices, m = 1, power = 1, directed = FALSE)
  G = asNetwork(G)
  ba.sim[[i]] <- G
}
...
setwd("..")
setwd("Data")
cell.sim = read.csv("CELL_graphs_from_barabasi.csv", header = FALSE)
...
ggsave("BA_comnb_power-n100.jpg", plot = powerplot)
write.csv(df,"BA_comnb_power-n100.csv", row.names = FALSE)
write.csv(rejected_indices_df,"BA_comnb_rejected_batches-n100.csv")