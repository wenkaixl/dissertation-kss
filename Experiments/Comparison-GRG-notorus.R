...
### EXPERIMENT
coef.h0 = 0.3
coef.h1.list = seq(0.1, 0.5, 0.025)
...
for (i in 1:n_sim_h0) {
  G = sample_grg(nodes = vertices, radius = coef.h0, torus = FALSE, coords = FALSE)
  G = asNetwork(G)
  g.sim[[i]] <- G
}
...
powertest.parallel <- function(coef_idx, kernel_idx, B_idx) {
  ...
  coef.h1 = coef
  g.sim.h1 <- lapply(rep(vertices, n_sim_h1), sample_grg, radius = coef.h1, torus = FALSE, coords = FALSE)
...
powerplot <- ggplot(data=df) + aes(x = Coefficient, y = Power, group = Kernel, colour = Kernel)+ geom_point() + geom_line() + ggtitle("Power of various kernels in GRG experiment (true coeff. = 0.3)")
print(df)

setwd("..")
setwd("Data")
#setwd("Geometric RG Experiment (no torus)")
ggsave("GRGnt_comnb_all_v20_B200_n500.jpg", plot = powerplot)
write.csv(df,"GRGnt_comnb_all_v20_B200_n500.csv", row.names = FALSE)