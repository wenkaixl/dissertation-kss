...
g.sim <- simulate(model0, nsim=n_sim_h0,
                  coef=coef,control=control.simulate(MCMC.burnin=1000+10*vertices, MCMC.interval=vertices))

setwd("..")
setwd("Data")
cell.sim = read.csv("CELL_graphs_from_E2S.csv", header = FALSE)

### EXPERIMENT

#t_fun based on (deg(i), deg(j))-lookup table
t_fun_exact = function(x){
  Del = coef[1] + coef[2]*del_t2s(x)
  S = sigmoid(Del)
}
...
powertest.parallel.cell <- function(i, kernel_idx, B_idx) {
  ...
  B = B.list[B_idx]
  res = perform.test.cell.sample(g.sim, GKSS.sampled, kernel = kernel[[1]], level = kernel[[2]], model0, coef.h1, n_sim_h1, B, sim_gkss[kernel_idx, B_idx, i])
...
for (kernel_idx in (1:length(kernels))) {
  for (B_idx in (1:length(B.list))) {
    ...
    rejections <- mclapply(as.list(1:n_sim_h1), mc.cores = 16, powertest.parallel.cell, kernel_idx=kernel_idx, B_idx=B_idx)
...
ggsave("E2S_exact_power-n100.jpg", plot = powerplot)
write.csv(df,"E2S_exact_power-n100.csv", row.names = FALSE)
write.csv(rejected_indices_df,"E2S_exact_rejected_batches-n100.csv")